...
#
# Implementa el algoritmo "bosque aleatorio" de Breiman para clasificación y regresión. 
# También se puede utilizar en el modo "sin supervisión" para evaluar proximidades entre 
# los puntos o datos
#
#  Orden:
...
#                ifual al número de estratos, entonces el muestreo es estratificado por "strata"
...
bag.spam$type # Clasificación o regresión

# bag.spam$predicted proporciona las clases asignadas según el método out-of-bag
bag.spam$predicted[1:5]

# bag.spam$err.rate proporciona (sólo para clasificación) el vector de las tasas de error  
#                   de la predicción en los datos de entrada, el elemento i-ésimo es la
#                   tasa de error OOB acumulado hasta el i-ésimo árbol generado
dim(bag.spam$err.rate)
bag.spam$err.rate[1:5,]

bag.spam$confusion # (sólo para clasificación) matriz de confusión de la predicción, 
                   #basado en el método OOB

# bag.spam$votes    (sólo para clasificación) matriz con una fila para cada caso 
#                    y una columna para cada clase, proporcionando la fracción o el número 
#                    de votos (OOB) del bosque aleatorio. Si norm.votes="FALSE", proporciona
#                    los totales
dim(bag.spam$votes)
bag.spam$votes[1:5,]

# bag.spam$oob.times número de veces que los casos son 'OOB' (y por lo tanto se utiliza en el cálculo
#                    de estimación del error OOB)
bag.spam$oob.times[1:5]

bag.spam$classes   # Clases del estudio

bag.spam$importance # una matriz con NClass+2 (para clasificación) o 2 (para regresión) columnas y
                    # tantas filas como variables explicativas
                    # Para la clasificación, las NClass primeras columnas son las medidas específicas
                    # de cada clase calculadas como media del "decrecimiento" de la precisión respecto
                    # a las estimacione basadas en permutaciones. 
                    # La columna NClass + primera es la disminución media en la precisión en todas 
                    # las clases debido a la variable explicativa asociada. 
                    # La última columna es la disminución media en el índice de Gini. 
                    # Para la regresión, véase el manual

bag.spam$importanceSD # Errores estandar de la medida de importancia basada en permutaciones

bag.spam$ntree      # número de árboles realizados
bag.spam$mtry       # númnero de predictores muestreados utilizados en la construcción de cada árbol
...
# cutoff   : (Sólo para clasificación) Vector de longitud igual al número de 
#             clases. La clase "ganadora" para una observación es la que tiene 
#             la máxima proporción de votos mayor que cutoff. Por defecto se toma 
#             los valores proporcionados, en su caso, en la orden de creación
